...
enrich = function(markers, outfilename, plot_title) {
  found_GO = TRUE
  found_KEGG = TRUE
  ...
  if (dim(do.call(rbind, bootstrap))[1] > 0) {
    ...
    print(keggEnrichment$Pathway_Name)
    
    keggEnrichment <- keggEnrichment[keggEnrichment$Pathway_Name %in% c(
  ...
  if (found_GO && found_KEGG) {
    pdf(
      paste(
        "../docs/plots/b_corrected/3.pathways/",
        outfilename,
        "_enriched_genes.pdf",
        sep = ""
      ),
    ...
    grid.arrange(p1, p2, ncol = 2, top = textGrob(
      paste(plot_title, "enriched genes"),
